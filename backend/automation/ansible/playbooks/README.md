...
This directory (`backend/automation/ansible/playbooks/`) contains Ansible playbooks used by the Homelab Manager for device automation and management.
...
**Use case**: Initial server provisioning.
...
- `GET /api/automation/executors` - List available executor types
- `GET /api/automation/executors/ansible/actions` - List available playbooks
- `POST /api/automation` - Trigger a playbook execution
- `GET /api/automation/{job_id}` - Get job status and logs
