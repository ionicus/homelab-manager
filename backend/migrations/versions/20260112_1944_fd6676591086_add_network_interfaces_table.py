...
down_revision: str | Sequence[str] | None = "5fea6ecb8e6a"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("UP", "DOWN", "DISABLED", name="interfacestatus"),
            nullable=True,
        ),
    ...
    op.create_index(
        op.f("ix_network_interfaces_id"), "network_interfaces", ["id"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        op.f("ix_network_interfaces_is_primary"),
        "network_interfaces",
        ["is_primary"],
        unique=False,
    )
    ...
    for device_id, ip_address, mac_address in devices:
        connection.execute(
            sa.text(
                ...
                "is_primary, status) "
                "VALUES (:device_id, :interface_name, :mac_address, "
                ":ip_address, :is_primary, :status)"
            ),
            {
                "device_id": device_id,
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(
        op.f("ix_network_interfaces_is_primary"), table_name="network_interfaces"
    )
    op.drop_index(
        op.f("ix_network_interfaces_mac_address"), table_name="network_interfaces"
    )
